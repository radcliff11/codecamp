...
then
  ...
  then
    # Non numeric
    ELEMENT=$($PSQL "SELECT atomic_number, atomic_mass, melting_point_celsius,boiling_point_celsius, symbol,name, type FROM properties JOIN elements USING(atomic_number) JOIN types USING(type_id) WHERE name LIKE '$1%' ORDER BY atomic_number LIMIT 1;")
  else
    # Numeric
    ...
    then
      ...
      echo $ELEMENT | while IFS=\| read ATOMIC_NUMBER ATOMIC_MASS MP BP SYMBOL NAME TYPE
      do
